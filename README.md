...
- `hero.jpg`（2000×1500）首頁滿版開場，ISLS 2026 於 UC Irvine 的會場照。
  人在右側、左邊開闊，姓名壓在左下，構圖比照參考站。
- `portrait.jpg`（864×1184）棚拍形象照，置於關於頁自述區右側。
- `gallery-1` 到 `gallery-3`（3:4 直式）關於頁末的側記，交錯排列。
...
2. 以 gamma 把中位亮度推到 158
3. 錨定黑白點到 26 / 250
4. 二分法解飽和倍率至 0.140，**只降不升**——低彩度的棚拍硬推只會讓膚色失真
5. 中間調微暖、極輕柔光
